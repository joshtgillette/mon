...
The names are not yours to choose, and they are the same under both runners: `JEN_GH_APP_ID_<ROLE>`, `JEN_GH_INSTALLATION_<ROLE>`, and `JEN_GH_PRIVATE_KEY_<ROLE>` for each of `DESIGN`, `DEV`, and `DELIVER`, plus `LINEAR_API_KEY` and one model credential. Eleven. The scheduled workflow reads them as repository secrets under exactly those names — it is written that way and jen rewrites it, so a secret stored under any other name reaches nothing — and `jen watch` reads the same names from the environment it was started in.
